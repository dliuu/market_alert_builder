...
def test_rsi_first_value_is_at_the_window_boundary() -> None:
...
def test_rsi_known_answers(offset: int, expected: str) -> None:
...
def test_rsi_returns_empty_below_window() -> None:
    assert rsi_series(SERIES[:RSI_WINDOW]) == []


def test_macd_hist_first_value_is_at_the_warmup_boundary() -> None:
...
def test_macd_hist_is_line_minus_signal(offset: int) -> None:
...
def test_macd_hist_returns_empty_below_warmup() -> None:
    assert macd_hist_series(SERIES[:33]) == []


def test_adx_first_value_is_at_the_warmup_boundary() -> None:
...
def test_adx_inside_bar_contributes_no_directional_movement() -> None:
...
def test_adx_outside_bar_credits_only_the_larger_move() -> None:
    prev = TechBar(date(2026, 1, 5), Decimal("50"), Decimal("45"), Decimal("48"), 1)
...
def test_atr_series_last_value_equals_m19_atr() -> None:
...
def test_series_are_deterministic_across_prefixes() -> None:
...
def test_percentile_needs_a_full_baseline() -> None:
...
def test_today_is_excluded_from_its_own_baseline() -> None:
    ...
    # slid to include it.
    diluted = percentile([*history[1:], outlier, outlier])
    assert diluted is not None
    assert diluted < Decimal(100)


def test_percentile_uses_only_the_last_252_prior_values() -> None:
    ancient = [Decimal(10_000)] * 50
    recent = [Decimal(1)] * BASELINE_SESSIONS
    assert percentile([*ancient, *recent, Decimal(2)]) == Decimal(100)


def test_read_depth_covers_the_longest_warmup() -> None:
...
def test_rel_strength_is_the_return_difference() -> None:
...
def test_rel_strength_empty_when_benchmark_is_short() -> None:
...
def _long_bars(n: int) -> list[TechBar]:
    ...
    closes = [
        Decimal(100) + Decimal(i) / Decimal(10) + (Decimal(i % 7) / Decimal(4))
        for i in range(n)
    ]
    return _bars([c.quantize(Decimal("0.01")) for c in closes])


def test_standing_with_full_history_populates_every_pair() -> None:
...
def test_standing_with_short_history_keeps_values_and_nulls_percentiles() -> None:
...
def test_standing_at_285_and_286_bars_is_the_percentile_boundary() -> None:
    short = standing_for_symbol(
        "A", _long_bars(285), benchmark_bars=_long_bars(285), benchmark_symbol="SPY"
    )
    exact = standing_for_symbol(
        "A", _long_bars(286), benchmark_bars=_long_bars(286), benchmark_symbol="SPY"
    )
    assert short.macd_hist_pctile is None
    assert exact.macd_hist_pctile is not None


def test_standing_without_a_benchmark_nulls_both_relative_fields() -> None:
    bars = _long_bars(READ_DEPTH)
...
def _bars_from(pairs: list[tuple[str, str, str]]) -> list[TechBar]:
    ...
    day = date(2026, 1, 5)
    return [
        TechBar(day + timedelta(days=i), Decimal(hi), Decimal(lo), Decimal(c), 1_000_000)
        for i, (hi, lo, c) in enumerate(pairs)
    ]
...
def test_bearish_divergence_fires_on_higher_high_with_lower_rsi() -> None:
...
def test_higher_high_with_higher_rsi_fires_nothing() -> None:
...
def test_bullish_divergence_fires_on_lower_low_with_higher_rsi() -> None:
    pairs: list[tuple[str, str, str]] = []
    for _ in range(6):
...
def test_pivots_closer_than_the_minimum_gap_fire_nothing() -> None:
    """gap=4 yields two real swing highs at bar indices 6 and 10 (actual gap 4
    < DIVERGENCE_MIN_GAP 5) — not gap=3, which collapses to a single pivot via
    swing_pivots' own k=3 window overlap and never reaches the gap comparison
    at all. RSI is pinned at those two indices to describe a shape that WOULD
    be a bearish divergence (higher high, lower RSI) if the gap bound were
    absent, so only the gap bound is what stops this."""
    bars = _peak_series("130", "135", 4)
    rsi = rsi_series([b.c for b in bars])
    assert divergence(bars, rsi, _rsi_at={6: Decimal("80"), 10: Decimal("60")}) is None


def test_pivots_further_than_the_maximum_gap_fire_nothing() -> None:
    """Mirror of the min-gap test above, at the far end: pivots at bar indices
    6 and 71 (actual gap 65 > DIVERGENCE_MAX_GAP 60), RSI pinned to describe a
    shape that would otherwise be a bearish divergence."""
    bars = _peak_series("130", "135", DIVERGENCE_MAX_GAP + 5)
    rsi = rsi_series([b.c for b in bars])
    assert divergence(bars, rsi, _rsi_at={6: Decimal("80"), 71: Decimal("60")}) is None


def test_fewer_than_two_pivots_fires_nothing() -> None:
    assert divergence(_bars_from([("100", "100", "100")] * 20), []) is None


def _bearish_divergence_bars() -> list[TechBar]:
    """A price path with a real (unpinned) bearish divergence: a strong 20-bar
    ramp to a swing high at 130, a pullback, then a SLOWER 30-bar ramp to a
    higher swing high at 135. The higher high is real; the weaker second ramp
    genuinely produces a lower RSI there under Wilder's smoothing — no
    `_rsi_at` seam needed. Confirmed by direct computation: swing_pivots finds
    highs at bar indices 20 and 60 (gap 40, inside DIVERGENCE_MIN_GAP..MAX_GAP)
    and rsi_series shows a lower RSI at the second."""
    ramp1 = [Decimal(100) + Decimal(30) * Decimal(i) / Decimal(20) for i in range(21)]
    pullback = [Decimal(130) - Decimal(15) * Decimal(i) / Decimal(10) for i in range(1, 11)]
    ramp2 = [Decimal(115) + Decimal(20) * Decimal(i) / Decimal(30) for i in range(1, 31)]
    tail = [Decimal(135) - Decimal(i) for i in range(1, 8)]
    closes = [c.quantize(Decimal("0.01")) for c in ramp1 + pullback + ramp2 + tail]
    day = date(2026, 1, 5)
    return [
        TechBar(
...
def test_standing_carries_the_divergence() -> None:
    """Behavioural, not a wiring-only check: this fixture's real (unpinned)
    RSI genuinely diverges bearish, so a regression to the Task-2 placeholder
    `divergence=None` — which `in (None, "bullish", "bearish")` would have let
    through silently — fails this assertion."""
    bars = _bearish_divergence_bars()
    s = standing_for_symbol("X", bars, benchmark_bars=[], benchmark_symbol=None)
    assert s.divergence == "bearish"
